...
export const TOOL_METADATA: Record<string, ToolMetadata> = {
  ...
  webDatabaseService: {
    ...
    inputSchema: { productId: z.string().describe("Product ID to check.") },
  },
  ...
  pimService: {
    ...
    inputSchema: { styleId: z.string().describe("Style/Product ID to look up in PIM.") },
  },

  dlqService: {
    name: "checkDeadLetterQueue",
    description: "Checks if a previous sync job failed and left a message in the DLQ.",
    inputSchema: { productId: z.string().describe("Product ID to check in DLQ.") },
  },
  ...
  guideService: {
    ...
    inputSchema: { errorCode: z.string().describe("Error code to look up.") },
  },
  ...
  verificationService: {
    ...
    inputSchema: { productId: z.string().describe("Product ID to verify.") },
  },
  ...
  memoryListService: {
    ...
    inputSchema: { productId: z.string().optional().describe("Product ID filter. Omit to retrieve all.") },
  },

  memoryDeleteService: {
    name: "deleteMemory",
    description: "Deletes a specific episodic memory entry by its ID. Use only when a memory is stale or incorrect.",
    inputSchema: { memoryContentId: z.string().describe("Memory record ID to delete.") },
  },
