...
/**
 * L2 Detective Sub-Agent
 *
 * Exposes a single MCP tool: delegateToL2Detective
 * Used by the main agent for root cause analysis of persistent failures.
 */
...
export const logic = async (input: any, { correlationId }: { correlationId: string }) => {
  ...
  // Sub-Agent configuration moved inside logic to prevent state leaks across warm invocations
